...
def _logo_src() -> Optional[str]:
    """Devuelve src para <img>: URL (si LOGO_URL) o data:URI (si LOGO_PATH)."""
    url = (st.secrets.get("LOGO_URL") or os.getenv("LOGO_URL"))
    if url:
        return str(url)
    p = (st.secrets.get("LOGO_PATH") or os.getenv("LOGO_PATH"))
    if not p:
        for c in ("assets/logo.png","assets/logo.jpg","assets/logo.svg","logo.png","logo.jpg","logo.svg"):
            if _PathForLogo(c).exists():
                p = c; break
    if p and _PathForLogo(p).exists():
        mime = _guess_mime(p) or "image/png"
        data = _PathForLogo(p).read_bytes()
        return f"data:{mime};base64,{_b64.b64encode(data).decode('utf-8')}"
    return None

def show_corner_logo(size_px: int = 42, top_px: int = 8, right_px: int = 12):
    src = _logo_src()
    if not src:
        return
    st.markdown(f"""
    <style>
    .corner-logo{{position:fixed;top:{top_px}px;right:{right_px}px;z-index:10000;pointer-events:none;}}
    .corner-logo img{{width:{size_px}px;height:{size_px}px;object-fit:contain;border-radius:8px;opacity:.95;}}
    </style>
    <div class="corner-logo"><img src="{src}" alt="logo"></div>
...
def login():
    ...
    # Logo fijo también en login (tamaño controlable por secrets)
    show_corner_logo(size_px=int(str(st.secrets.get("LOGO_SIZE_LOGIN","56"))))
...
# Logo fijo en la app (tamaño controlable por secrets)
show_corner_logo(size_px=int(str(st.secrets.get("LOGO_SIZE_APP","44"))))
